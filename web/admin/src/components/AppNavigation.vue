					...
					<template v-if="user && user.isLoggedIn === true">
						<li class="navbar-text">
							<router-link class="navbar-text text-decoration-none" :to="{ name: 'Profile', params: { uid: user.data.uid } }"
								...
								<i v-if="user.isVolunteer && user.data && user.data.emailVerified" class="bi bi-patch-check-fill text-success"></i
							></router-link>
						</li>
						<li v-if="user.isVolunteer && !(user.data && user.data.emailVerified)" class="navbar-text">
							<router-link
